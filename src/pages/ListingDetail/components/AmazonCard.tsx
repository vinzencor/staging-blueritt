...
const AmazonCard: React.FC<{ amazonProduct: TAmazonProduct }> = ({
  ...
  return (
    <SpkbgCards
      key={`product-${productData.asin || (productData as any).product_id || (productData as any).id || 'unknown'}`}
      mode="basic"
      Title={getTitle()}
      Price={isTikTokProduct ? getPrice() : (transformedAmazonProduct.price || 0)}
      Imgsrc={getImage()}
      ...
      SalesVolume={`${isTikTokProduct ? getSalesVolume() : (transformedAmazonProduct.salesVolume || 0)}`}
